...
function t7() {
    ...
    document.querySelector('.out-7').innerHTML = a7[index];
}
...
function t8(event) {
    document.querySelector('.out-8').innerHTML += (event.key in a8) ? a8[event.key] : event.key;
}
...
function t9(event) {
    if (event.key == 'ArrowDown') {
        document.querySelector('.out-9').innerHTML = arrow_down++;
    }
}
...
function t10(event) {
    let block = document.querySelector('.block-10').style;
