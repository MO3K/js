...
let ar6 = ['Vladimir', 'Taurus', 23, 'april'];
...
function f12() {
    let last_elem = ar12.length - 1;
    let last = ar12[last_elem]
    ar12[last_elem] = ar12[0];
    ar12[0] = last;
    let out = '';
    let i = 0;
...
function f16() {
    ...
    while (k1 < ar16_odd.length) {
        out1 += ar16_odd[k1] + ' ';
        k1++;
    }
    while (k2 < ar16_even.length) {
        out2 += ar16_even[k2] + ' ';
        k2++;
    }
...
function f17() {
    ...
    document.querySelector('.out-17').innerHTML = k;
}
