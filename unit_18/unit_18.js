...
function t4() {
    ...
    span.forEach(elem => a4_res.push(+elem.getAttribute('data')));
    console.log(a4_res);
}
...
function t7() {
    let str7 = 'hello world hi mazai'; //в задании, скорее всего ошибка - используется массив a6_res с предыдужего задания
    a7_res = str7.split(' ');
...
function t9() {
    let a9 = [['hi', 'mahai'], ['test', 'best']]; //ошибка - значение массива должны быть строками
    let arr = [];

    a9.forEach(elem => arr.push(elem.join('-')));
    a9_res = arr.join('-');
...
function t10() {
    let a10 = { name: "ivan", age: 15, sex: 1, id: 45 }; //ошибка - значение ivan должно быть строкой
    let get = '';
